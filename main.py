    ...
    while True:
        try:
            ...
            if send_response.status_code == 200:
                ...
                log_message("error", f"Token {nama_token} ({token[:10]}...): Gagal mengirim emoji: {send_response.status_code}, {send_response.text}")
                break
                ...
                if get_response.status_code == 200:
                    ...
                    if messages:
                        ...
                        if delete_response.status_code == 204:
                            ...
                            log_message("warning", f"Token {nama_token} ({token[:10]}...): Gagal menghapus pesan (percobaan {retries + 1}): {delete_response.status_code}, {delete_response.text}")
                            retries += 1
                            time.sleep(1)
                    else:
                    ...
                    log_message("error", f"Token {nama_token} ({token[:10]}...): Gagal mendapatkan pesan: {get_response.status_code}, {get_response.text}")
                    break
            ...
            log_message("error", f"Token {nama_token} ({token[:10]}...): Request error: {e}")
...
with ThreadPoolExecutor(max_workers=20) as executor:  # Batasi maksimal 20 token berjalan paralel
    for nama_token, token in tokens:
        executor.submit(kirim_pesan, channel_id, nama_token, token, emoji_list, waktu_hapus, waktu_kirim)
